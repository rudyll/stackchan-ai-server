...
body{margin:0;background:#10131a;color:#e8edf5;font:15px system-ui,-apple-system,sans-serif}main{max-width:850px;margin:auto;padding:24px}h1{font-size:24px}nav{display:flex;gap:8px;margin:20px 0}button{border:0;border-radius:8px;padding:10px 14px;background:#273246;color:#fff;cursor:pointer}button.active,button.save{background:#3d7eff}.panel{display:none;background:#18202d;padding:20px;border-radius:12px}.panel.active{display:block}.field{margin:13px 0}.field label{display:block;font-weight:650;margin-bottom:5px}.hint{font-size:12px;color:#a7b2c6;margin:4px 0}input,select,textarea{box-sizing:border-box;width:100%;padding:10px;border:1px solid #3c4960;border-radius:7px;background:#101722;color:#fff}textarea{min-height:110px;font-family:ui-monospace,monospace}.provider{display:none}.provider.show{display:block}.notice{margin:14px 0;color:#8de0ad}.warn{color:#ffd280}</style><main>
<h1>StackChan AI 设置</h1><p class="hint">保存后，新建对话会使用新设置；让设备断开重连即可立即切换。</p><nav><button class="tab active" data-tab="basic">基础</button><button class="tab" data-tab="voice">语音管线</button><button class="tab" data-tab="devices">设备 Profile</button></nav>
