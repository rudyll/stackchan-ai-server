...
function showCatalog(result){const box=q('#catalog');box.replaceChildren();const title=document.createElement('strong');title.textContent='检测结果';box.appendChild(title);if(result.error){const p=document.createElement('p');p.className='error';p.textContent=result.error;box.appendChild(p)}else{const p=document.createElement('p');p.className='notice';p.textContent='Provider API 可访问。模型 '+result.models.length+' 个；声音 '+result.voices.length+' 个。';box.appendChild(p);if(result.warnings){result.warnings.forEach(w=>{const x=document.createElement('p');x.className='hint';x.textContent=w;box.appendChild(x)})}const field=name=>all('[name]').find(e=>e.name===name);const modelField=field(result.model_key),voiceField=field(result.voice_key);const modelIDs=new Set((result.models||[]).map(item=>item.id)),voiceIDs=new Set((result.voices||[]).map(item=>item.id));if(modelField&&modelField.value&&result.models.length&&!modelIDs.has(modelField.value)){const x=document.createElement('p');x.className='warn';x.textContent='当前模型未出现在 Provider 返回列表中，可能无权限或不是该 Provider 支持的模型。';box.appendChild(x)}if(voiceField&&voiceField.value&&result.voices.length&&!voiceIDs.has(voiceField.value)){const x=document.createElement('p');x.className='warn';x.textContent='当前声音不在当前 Provider 的内置目录中，请确认名称。';box.appendChild(x)}const m=document.createElement('div');m.innerHTML='<b>可用模型</b> ';(result.models||[]).slice(0,30).forEach(item=>{const tag=document.createElement('span');tag.className='tag';tag.textContent=item.id;m.appendChild(tag)});box.appendChild(m);const v=document.createElement('div');v.innerHTML='<b>可用声音</b> ';(result.voices||[]).forEach(item=>{const tag=document.createElement('span');tag.className='tag';tag.textContent=item.id;v.appendChild(tag)});box.appendChild(v)}fillList('model-options',result.models);fillList('voice-options',result.voices)}
