...
默认情况下，语音 WebSocket 服务发布在宿主机 `12800` 端口（容器内部仍监听 `12800`）；设置页默认只绑定 `127.0.0.1:8099`。首次启动日志会显示 Bearer token，并持久化到 `./data/settings-token`。GUI 提供 OpenAI Realtime、Gemini Live、TokenHub、OpenRouter 和 OpenAI-compatible 独立入口，并可检测 Provider、获取模型名称和填充对应的声音目录；同时可以切换 Gemini HA tools/Search。两者互斥，standalone 用户启用 Search 前应先关闭 HA tools。standalone 不连接 Home Assistant，也不启动 443 端口 OTA 劫持；设备 OTA 地址配置为 `http://<服务器局域网IP>:12800/xiaozhi/ota/`。
